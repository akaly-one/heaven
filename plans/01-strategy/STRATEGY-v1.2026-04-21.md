...
> **Document maître confidentiel — vision stratégique 2026-2027**
...
| Doc | Rôle |
|---|---|
| **`business/bp-agence-heaven-2026-04/`** | **BP v1 source de vérité business** (4 docs : BP Word 14 p., modèle financier 24 mois, paliers rémunération, Release Form) |
| `ROADMAP-MISE-A-NIVEAU-AVRIL-2026.md` | Roadmap d'exécution technique courante (modules A-I, décisions D-1/D-6) |
| `product/objectifs.md` | KPIs produit alignés BP §10 |
| `product/roadmap.md` | Sprints S1-S7 d'implémentation BP |
| `product/modules.md` | Catalogue modules CP (existants + dérivés BP) |
| `tech/architecture.md` | Single Next.js, DB ~22 tables, queue/worker |
| `security/roles-entities.md` | Rôles + scopes DMCA/DPO/Palier |
| `REFACTOR-NAVIGATION-SPEC.md` | Refonte navigation (D-1/D-2/D-3) |
| `META-APP-PUBLICATION-PLAN.md` | Business Verification + App Review (D-4) |
| `IA-AGENT-SPEC.md` | Agent IA persona + prompt + coûts (D-5) |
| `MIGRATION-2026-04.md` | Log 9 migrations SQL 030-038b |
| `MAINTENANCE-PREVENTIVE.md` | Cadences + règles absolues |

---

## 🔥 État avril 2026 (21/04)

**Infra plateforme livrée** : `heaven-os.vercel.app` v1.1 — auth JWT, DB Supabase Heaven isolée (22 tables), Cloudinary edge cache 30j, 9 migrations appliquées, messagerie unifiée web+IG, feed polymorphe, queue worker IG, observabilité 6 KPIs, Meta App Review artifacts déployés.

**Bloquants business actuels** (décisions NB pending) :
- D-4 Business Verification SQWENSY BE → unlock App Review Meta → IG DMs prod
- D-5 clé IA OpenRouter → activer agent review mode
- D-6 infra cron (Vercel Pro / Upstash QStash / GitHub Actions) → worker 1/min
- D-1/D-2/D-3 refonte navigation cockpit

---

## TL;DR exécutif — 3 Modes opérationnels

Heaven opère **3 modes de fonctionnement** indépendants sur **une même infrastructure** (Sqwensy OS + CP + agent IA + automations).

| Mode | Activité | Revenu Sqwensy | Ouverture |
|---|---|---|---|
| **A — Studio IA pur** | Personas 100 % IA sur comptes agence (Fanvue `yumiclub` + IG `@Yumiiiclub` + TikTok + Snap) | **100 %** après coûts prod IA | **T1 (M1-M3)** |
| **B — Hub annexe modèles** | Modèles réelles publiées sur comptes agence via Release Form DMCA | **30 %** du net distribuable (70 % modèle) | **T2 (M4-M6)** |
| **C — Services B2B** | Sqwensy = prestataire tech/stratégique, modèle reste 100 % indépendante | Setup 800-2 500 € + sub 150-500 € + 5-10 % croissance | **T4 (M10-M12)** |

**Deux standards transversaux** (Modes B + C) : Plan Identité **Découverte** (visage assumé) ou **Shadow** (visage caché). Protection identité légale non-négociable dans les deux.

**Caming = canal d'acquisition primaire** (Stripchat/Bongacams/Chaturbate) pour Mode B. Sessions live → redirection UTM → pages PPV Fanvue. Attribution J+7.

**Objectifs chiffrés consolidés** :
- M3 : ≥ 500 abonnés free Fanvue + ≥ 100 € cumul
- M6 : ≥ 2 modèles B + ≥ 400 €/mois A+B
- M9 : ≥ 750 €/mois A+B + marge Sqwensy ≥ 150 €
- M12 : ≥ 1 150 €/mois A+B + 1 cliente Mode C

Scénario optimiste T4 : > 2 500 €/mois A+B → scale.
Scénario pessimiste T4 : < 500 €/mois → sortie ou pivot.

**Stack IA content budgété** : 60 €/mois Q2 → 160 €/mois Q4 → 380 €/mois Q4 2027. Paie via carte Wise virtuelle « Heaven ».

**Confidentialité absolue** : Heaven ne doit JAMAIS être associé publiquement à SQWENSY. Aucun vrai prénom en code/doc/DB/commits. IDs internes : `m1`=yumi, `m2`=paloma, `m3`=ruby.

**Paliers rémunération modèles Mode B (P1-P4 BE)** : P1 Test < 1 k€ (droit à l'image), P2 Démarrage 1-9 k€ (droit à l'image), P3 Structuration 9-20 k€ (indép complémentaire OBLIGATOIRE), P4 Pro > 20 k€ (TVA si > 25 k€). Bascule auto 3 mois > 750 €.

**Risques critiques (BP §8)** :
- Ban Fanvue agence (existentielle) → DMCA strict + backup Fansly 2 sem
- Modèle chômage non déclarée (existentielle) → check ONEM obligatoire avant signature
- AI Act UE 2 août 2026 → label « AI-generated » bio + tag post tous contenus Mode A
- Conversion PPV < 2 % (élevée) → pivot abo 9,99 $/mois M6
...
> **Note cadrage** : La vision initiale 2 hubs (« YUMI IA » + « NB profil humain ») est **remplacée** par le cadre **3 Modes** du BP v1 avril 2026 (cf. `business/bp-agence-heaven-2026-04/`). La section ci-dessous conserve la qualification produit/niche ; la structure économique et opérationnelle suit les 3 Modes A/B/C.

Heaven ne joue **pas sur le volume** (100 modèles). Heaven joue sur la **qualité curation + premium + dualité IA/humaine** :

- **Mode A = Studio IA pur** — YUMI (m1) positionnée AI influenceuse premium mode/lifestyle style Aitana Lopez **mais classy pure** (pas fitness racoleur, pas hard explicite sur Insta, seulement classe + sublime + émotions joie). Compte Fanvue `yumiclub` + IG `@Yumiiiclub` au nom de l'agence.
- **Mode B = Hub annexe modèles réelles** — PALOMA (m2) et RUBY (m3) publiées sur comptes agence via Release Form DMCA. 70 % modèle / 30 % Sqwensy. Plan Identité au choix : Découverte ou Shadow.
- **Mode C = Services B2B** — Sqwensy prestataire pour modèles externes indépendantes (setup + sub + % croissance).
- **Cross-promotion inter-Modes** = maximise LTV fan (un fan qui aime YUMI Mode A peut explorer les contenus Mode B via teaser, et inversement).
...
**Scénario bull (Modes A+B+C plein régime)** : Mode A YUMI 100k Insta Q4 2027, référence IA influenceuse EU. Mode B 4-5 modèles actives stables. Mode C 3-5 clientes B2B. MRR consolidé > 4 000 €/mois. Décision fin 2027 : SRL Heaven séparée ou continuation branche Agence ? Si SRL : ouvrir SaaS Heaven externe.

**Scénario nominal (Modes A+B)** : YUMI 50k Insta, CA A+B 2 500-3 500 €/mois, 2-3 modèles Mode B actives. ROI annuel 30-40 k€. Rester branche Agence silo. Mode C repoussé.

**Scénario stress** : Instagram ban Mode A YUMI, AI Act non respecté → pivot sur Mode B uniquement (100 % modèles réelles), diversification TikTok/X.
