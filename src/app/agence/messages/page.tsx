...
import { MessageSquare, Send, Trash2, Ban, Search, ArrowLeft } from "lucide-react";
    ...
    <OsLayout cpId="agence">
      <div className="min-h-screen p-4 md:p-8 pb-24 md:pb-8">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[320px_1fr] gap-4 fade-up-1" style={{ minHeight: "calc(100vh - 200px)" }}>
              ...
              <div className="flex-1 overflow-y-auto">
                {loading ? (
                  ...
                  <button key={conv.client.id} onClick={() => setSelectedClient(conv.client.id)}
                    ...
                    style={{
                      background: selectedClient === conv.client.id ? "rgba(99,102,241,0.06)" : "transparent",
                      borderBottom: "1px solid var(--border2)",
                    }}>
                    <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 text-xs font-bold"
                      style={{
                        background: conv.client.is_verified ? "rgba(16,185,129,0.12)" : "rgba(99,102,241,0.12)",
                        color: conv.client.is_verified ? "var(--success)" : "var(--accent)",
                      }}>
                    ...
                    {[...selectedConv.messages].reverse().map(msg => (
                      <div key={msg.id} className={`flex ${msg.sender_type === "model" ? "justify-end" : "justify-start"}`}>
                        <div className="max-w-[75%] rounded-2xl px-4 py-2.5 text-xs relative group"
                          style={{
                            background: msg.sender_type === "model" ? "rgba(99,102,241,0.12)" : "var(--bg3)",
                            color: "var(--text)",
                          }}>
                          <p>{msg.content}</p>
                  ...
                  <div className="p-3" style={{ borderTop: "1px solid var(--border2)" }}>
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={handleSend} disabled={!reply.trim()}
                        className="w-9 h-9 rounded-xl flex items-center justify-center cursor-pointer btn-gradient disabled:opacity-30">
